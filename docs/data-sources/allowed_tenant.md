...
`name` - (Required) Name of the AllowedTenant (`String`).

`namespace` - (Required) Namespace where the AllowedTenant exists (`String`).
...
`description` - (Optional) Description of the AllowedTenant (`String`).
