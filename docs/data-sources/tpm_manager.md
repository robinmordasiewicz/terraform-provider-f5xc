...
`name` - (Required) Name of the TpmManager (`String`).

`namespace` - (Required) Namespace where the TpmManager exists (`String`).
...
`description` - (Optional) Description of the TpmManager (`String`).
