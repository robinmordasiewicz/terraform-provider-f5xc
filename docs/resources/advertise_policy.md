---
page_title: "f5xc\_advertise\_policy Resource - terraform-provider-f5xc"
...
# f5xc\_advertise\_policy (Resource)
...
resource "f5xc\_advertise\_policy" "example" {
  ...
  client\_certificate\_optional {
    # Configure client\_certificate\_optional settings
  }
}
```
...
<a id="skip-xff-append"></a>&#x2022; [`skip\_xff\_append`](#skip-xff-append) - Optional Bool<br>Disable X-Forwarded-For Header. If set, the loadbalancer will not append the remote address to the x-forwarded-for HTTP header
...
terraform import f5xc\_advertise\_policy.example system/example
```
