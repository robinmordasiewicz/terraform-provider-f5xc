...
`aws_parameters` - (Optional) AWS Service VPC and TGW. Setup AWS services VPC, transit gateway and site.
...
`tgw_security` - (Optional) TGW Security Configuration. Security Configuration for transit gateway.

`timeouts` - (Optional)

`vn_config` - (Optional) Virtual Network Configuration. Virtual Network Configuration.

`vpc_attachments` - (Optional) Spoke VPCs. Spoke VPCs to be attached to the AWS TGW Site.
