...
`group_assignments` - (Optional) Group Mapping. The Group Mapping field is used to associate local user groups with user groups in child tenants. ves.io.schema.rules.repeated.max_items: 16 ves.io.schema.rules.repeated.unique: true. See [Group Assignments](#group-assignments) below for details.
...
`child_tenant_groups` - (Optional) Child Tenant User Groups. List of group names in child tenant. Note - To establish access, child tenant group names must be a subset of child tenant groups configured in tenant profile (`List`).
