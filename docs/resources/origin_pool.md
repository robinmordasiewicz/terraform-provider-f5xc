...
<br>`lb_port` - (Optional) Empty. This can be used for messages where no values are needed (`Block`).
<br>`port` - (Optional) Port. Endpoint service is available on this port (`Number`).
...
<br>`same_as_endpoint_port` - (Optional) Empty. This can be used for messages where no values are needed (`Block`).
...
<br>`use_tls` - (Optional) TLS Parameters for Origin Servers. Upstream TLS Parameters (`Block`).
