...
&#x2022; `name` - Required String<br>Name of the TpmAPIKey. Must be unique within the namespace

&#x2022; `namespace` - Required String<br>Namespace where the TpmAPIKey will be created
...
&#x2022; `category_ref` - Optional Block<br>TPM Category. APIKey needs a reference to an existing TPM category object TPM category is used for grouping customer owned APIKeys and manage them as one entity<br>See [Category Ref](#category-ref) below for details.

&#x2022; `need_mtls` - Optional Bool<br>Need mTLS. is mTLS required when using this APIKey?
