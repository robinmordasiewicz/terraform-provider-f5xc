...
func SetupMockTest(t *testing.T) *MockTestConfig {
	...
	os.Setenv("F5XC_API_URL", server.URL())
	os.Setenv("F5XC_API_TOKEN", "mock-token")
...
func (m *MockTestConfig) Cleanup() {
	...
	if m.OriginalURL != "" {
		os.Setenv("F5XC_API_URL", m.OriginalURL)
	} else {
		os.Unsetenv("F5XC_API_URL")
	}

	if m.OriginalToken != "" {
		os.Setenv("F5XC_API_TOKEN", m.OriginalToken)
	} else {
...
func GetGlobalMockServer() *mocks.Server {
	globalMockServerOnce.Do(func() {
		...
		os.Setenv("F5XC_API_URL", globalMockServer.URL())
		os.Setenv("F5XC_API_TOKEN", "mock-token")
		// Clear P12/PEM credentials so the provider uses token auth
		os.Unsetenv("F5XC_API_P12_FILE")
		os.Unsetenv("F5XC_P12_PASSWORD")
		os.Unsetenv("F5XC_API_CERT")
		os.Unsetenv("F5XC_API_KEY")
	})
