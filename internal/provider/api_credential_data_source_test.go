...
func TestAccApiCredentialDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_api_credential.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccApiCredentialDataSourceConfig_basic(nsName, rName),
...
data "f5xc_api_credential" "test" {
  depends_on = [f5xc_api_credential.test]
  name       = f5xc_api_credential.test.name
  namespace  = f5xc_api_credential.test.namespace
}
`, nsName, name))
}
