...
func TestAccBigipIruleDataSource_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccBigipIruleDataSourceConfig_basic(nsName, rName),
...
data "f5xc_bigip_irule" "test" {
  depends_on = [f5xc_bigip_irule.test]
  name       = f5xc_bigip_irule.test.name
  namespace  = f5xc_bigip_irule.test.namespace
}
`, nsName, name))
}
