...
func TestAccEndpointDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_endpoint.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccEndpointDataSourceConfig_basic(nsName, rName),
...
data "f5xc_endpoint" "test" {
  depends_on = [f5xc_endpoint.test]
  name       = f5xc_endpoint.test.name
  namespace  = f5xc_endpoint.test.namespace
}
`, nsName, name))
}
