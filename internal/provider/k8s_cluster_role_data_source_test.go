...
func TestAccK8sClusterRoleDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_k8s_cluster_role.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccK8sClusterRoleDataSourceConfig_basic(nsName, rName),
...
data "f5xc_k8s_cluster_role" "test" {
  depends_on = [f5xc_k8s_cluster_role.test]
  name       = f5xc_k8s_cluster_role.test.name
  namespace  = f5xc_k8s_cluster_role.test.namespace
}
`, nsName, name))
}
