...
func TestAccNfvServiceDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_nfv_service.test"
...
data "f5xc_nfv_service" "test" {
  depends_on = [f5xc_nfv_service.test]
  name       = f5xc_nfv_service.test.name
  namespace  = f5xc_nfv_service.test.namespace
}
`, nsName, name))
}
