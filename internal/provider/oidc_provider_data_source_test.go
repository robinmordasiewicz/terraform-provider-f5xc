...
func TestAccOidcProviderDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_oidc_provider.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccOidcProviderDataSourceConfig_basic(nsName, rName),
...
data "f5xc_oidc_provider" "test" {
  depends_on = [f5xc_oidc_provider.test]
  name       = f5xc_oidc_provider.test.name
  namespace  = f5xc_oidc_provider.test.namespace
}
`, nsName, name))
}
