...
func TestAccReportConfigDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_report_config.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccReportConfigDataSourceConfig_basic(nsName, rName),
...
data "f5xc_report_config" "test" {
  depends_on = [f5xc_report_config.test]
  name       = f5xc_report_config.test.name
  namespace  = f5xc_report_config.test.namespace
}
`, nsName, name))
}
