...
func TestAccSecuremeshSiteDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_securemesh_site.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSecuremeshSiteDataSourceConfig_basic(nsName, rName),
...
data "f5xc_securemesh_site" "test" {
  depends_on = [f5xc_securemesh_site.test]
  name       = f5xc_securemesh_site.test.name
  namespace  = f5xc_securemesh_site.test.namespace
}
`, nsName, name))
}
