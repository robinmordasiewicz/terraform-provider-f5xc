...
func TestAccSrv6NetworkSliceDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_srv6_network_slice.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSrv6NetworkSliceDataSourceConfig_basic(nsName, rName),
...
data "f5xc_srv6_network_slice" "test" {
  depends_on = [f5xc_srv6_network_slice.test]
  name       = f5xc_srv6_network_slice.test.name
  namespace  = f5xc_srv6_network_slice.test.namespace
}
`, nsName, name))
}
