...
func TestAccVoltstackSiteDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_voltstack_site.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVoltstackSiteDataSourceConfig_basic(nsName, rName),
...
data "f5xc_voltstack_site" "test" {
  depends_on = [f5xc_voltstack_site.test]
  name       = f5xc_voltstack_site.test.name
  namespace  = f5xc_voltstack_site.test.namespace
}
`, nsName, name))
}
