...
func TestAccWorkloadDataSource_basic(t *testing.T) {
	...
	dataSourceName := "data.f5xc_workload.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccWorkloadDataSourceConfig_basic(nsName, rName),
...
data "f5xc_workload" "test" {
  depends_on = [f5xc_workload.test]
  name       = f5xc_workload.test.name
  namespace  = f5xc_workload.test.namespace
}
`, nsName, name))
}
