...
Add to your opencode configuration (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
...
```json
{
  "mcp": {
    "f5xc-terraform": {
      "type": "local",
      "command": ["npx", "-y", "@robinmordasiewicz/f5xc-terraform-mcp"],
      "enabled": true
    }
  }
}
...
With environment variables for API access:
