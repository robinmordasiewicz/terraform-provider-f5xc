...
export async function handleAuth(input: AuthInput): Promise<string> {
  ...
  switch (operation) {
    ...
    case 'terraform-block':
      return handleTerraformBlock(response_format);
    default:
...
function formatShellExports(
  ...
  const lines: string[] = [
    ...
    '',
    '# CRITICAL: Use the correct provider source!',
    '# Provider: robinmordasiewicz/f5xc (NOT volterraedge/volterra)',
    '',
  ];
  ...
  // Append provider block as a comment for reference
  lines.push('');
  lines.push('# Required Terraform provider configuration:');
  lines.push('#');
  for (const providerLine of TERRAFORM_PROVIDER_BLOCK.split('\n')) {
...
function formatDotEnv(
  ...
  const lines: string[] = [
    '# F5XC Terraform Provider Configuration',
    `# Profile: ${profileName || 'environment'}`,
    '# CRITICAL: Use provider source robinmordasiewicz/f5xc (NOT volterraedge/volterra)',
    '',
  ];
  ...
  switch (outputType) {
    case 'shell':
      return [
        ...
        `**Auth Method**: ${authMode}`,
        '',
        '> 🚨 **CRITICAL**: Use provider source `robinmordasiewicz/f5xc` - NOT `volterraedge/volterra` (deprecated)',
        '',
        '```bash',
        formatShellExports(envVars, activeProfile, authMode),
        '```',
        '',
        '## ⚠️ EXECUTE BEFORE TERRAFORM',
        '',
        '**You MUST execute these export commands BEFORE running terraform!**',
        '',
        '1. Copy the export commands above',
        '2. Paste them into your terminal (or `source ./f5xc-env.sh`)',
        '3. THEN run `terraform init && terraform plan`',
        '',
        '**Skipping this step causes "Missing Authentication Configuration" error.**',
        '',
        '## Required Provider Configuration',
        '',
        '```hcl',
        TERRAFORM_PROVIDER_BLOCK,
        '```',
      ].join('\n');
    ...
    case 'dotenv':
      return [
        ...
        '```',
        '',
        '## ⚠️ LOAD BEFORE TERRAFORM',
        '',
        '**You MUST load these environment variables BEFORE running terraform!**',
        '',
        '1. Save to `.env` file',
        '2. Load with `export $(cat .env | xargs)` or use a dotenv tool',
        '3. THEN run `terraform init && terraform plan`',
        '',
        '**Skipping this step causes "Missing Authentication Configuration" error.**',
        '',
    ...
    case 'json':
      return [
        ...
        '```',
        '',
        '## ⚠️ SET BEFORE TERRAFORM',
        '',
        '**You MUST set these environment variables BEFORE running terraform!**',
        '',
        'Use the `shell_command` from above or set each variable individually.',
        '',
        '**Skipping this step causes "Missing Authentication Configuration" error.**',
        '',
        '## Required Provider Configuration',
...
// =============================================================================
// TERRAFORM-BLOCK OPERATION
// =============================================================================

/**
 * Handle 'terraform-block' operation - return the correct Terraform provider block
 *
 * This is a quick reference for AI assistants to get the exact provider configuration
 * without needing to process environment variables or credentials.
 */
function handleTerraformBlock(format: ResponseFormat): string {
  ...
  return [
    ...
    '## Links',
    '',
    `- **Registry**: ${result.registry_url}`,
    `- **GitHub**: ${result.github_url}`,
    '',
    '## 🔐 Authentication Required',
    '',
    'This provider requires authentication before terraform operations.',
    '',
    '**Workflow:**',
    '1. `f5xc_terraform_auth(operation="terraform-env")` → get credentials',
    '2. Set environment variables (`export F5XC_API_URL=...`, `export F5XC_API_TOKEN=...`)',
    '3. Then run `terraform plan`',
    '',
    '**Without authentication, terraform fails with "Missing Authentication Configuration".**',
  ].join('\n');
}
